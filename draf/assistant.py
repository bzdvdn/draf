...
conversation: it holds the compiled :class:`~draf.graph.Graph`, its tools,
the :class:`~draf.checkpoint.Checkpointer`, and the conversation state shape
...
* :meth:`Assistant.stream` — the streaming equivalent, yielding
  :class:`~draf.stream.StreamEvent` objects.
...
The turn machinery itself lives on :class:`~draf.graph.Graph`
...
from draf.graph import Graph, TurnResult
