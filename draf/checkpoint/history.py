...
A plain :class:`~draf.checkpoint.SQLiteCheckpointer` / ``PGCheckpointer``
...
from draf.checkpoint.base import DEFAULT_OWNER, Checkpoint
...
class _HistoryMixin:
    ...
    :class:`draf.checkpoint.sqlite.SQLiteCheckpointer`); the mixin's
...
class SQLiteHistoryCheckpointer(_HistoryMixin, SQLiteCheckpointer):
    ...
    A drop-in for :class:`draf.checkpoint.SQLiteCheckpointer` that, on every
...
class PGHistoryCheckpointer(_HistoryMixin, PGCheckpointer):
    ...
    Requires ``asyncpg`` (install via ``draf[pg-checkpoint]``).  Mirrors
