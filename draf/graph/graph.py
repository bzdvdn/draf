...
from draf.graph.edge import _INTERRUPT_KEY, Edge, Hook
...
class Graph:
    ...
    async def run(
        ...
        * **Conversation turn** (with *message*): the run is driven as
          ...
          :class:`~draf.assistant.Assistant` wrapper exposes as
          ``run``/``stream``.
    ...
    async def pending(
        ...
        *,
        ...
        on an :class:`~draf.node.Interrupt` it writes a ``__interrupt__`` entry
