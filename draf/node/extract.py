...
An :class:`Extract` is :class:`~draf.node.ask.Ask`'s sibling: instead of
...
:class:`~draf.node.LLM` with ``json_schema`` / ``output_type`` and optional
...
class Extract:
    ...
    threaded through to :class:`~draf.node.LLM`.
    """
