...
def _ensure_api_key(target: Path) -> None:
    """Write a fresh random ``DRAF_API_KEY`` into the project's local ``.env``.
    ...
    """
    env_file = target / ".env"
    if env_file.exists() and "DRAF_API_KEY=" in env_file.read_text(encoding="utf-8"):
        return
    with env_file.open("a", encoding="utf-8") as fh:
        fh.write("\n# Generated by `draf new`; required by the fail-closed API gate.\n")
        fh.write(f"DRAF_API_KEY={secrets.token_urlsafe(32)}\n")
