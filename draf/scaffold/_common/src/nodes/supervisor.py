...
_AGENTS = frozenset({"planner", "writer", "reviewer", "finish"})
...
class Supervisor(Node):
    ...
    (a single word), and writes ``next_agent``.  When there is no user
    message to route, or the round counter reached ``max_rounds``, the
