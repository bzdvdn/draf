...
``draf.testing`` is a companion for writing fast, offline tests of
graphs that contain :class:`~draf.node.llm.LLM` or
:class:`~draf.node.agent.ReActAgent` nodes.  It provides two layers:

- :class:`FakeLLM` — a drop-in :class:`~draf.node.Node` subclass that
...
- :func:`mock_llm` — a pytest fixture that patches
  :class:`~draf.harness.Harness` so *real* ``LLM`` / ``ReActAgent``
...
from draf.harness.loop import Harness
...
if pytest is not None:
    ...
    def mock_llm(monkeypatch):
        ...
        Patches :class:`~draf.harness.Harness` transport so any graph —
