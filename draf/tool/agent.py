...
An :class:`AgentTool` wraps a sub-agent (:class:`~draf.harness.Harness`)
...
the active :class:`~draf.node.context.ExecContext` (providers, tracer,
``on_llm_payload``) and threading the enclosing workflow ``state`` in via
the runtime ``__state__`` / ``__ctx__`` kwargs (see
:func:`draf.harness.tools`).  Subclasses only declare *content*: the system
...
from draf.schema import parse_json_object, validate_json
...
class AgentTool(Tool):
    ...
    :func:`draf.harness.tools`); anything :meth:`handle_reply` writes into
    *state* is copied back into the enclosing workflow state.
    ...
    def handle_reply(self, state: dict, reply) -> str:
        ...
        :class:`~draf.harness.loop.Step` (``reply.content`` holds the text).
    ...
    def _harness(self, ctx):
        ...
        from draf.harness import Harness  # local import breaks the tool↔harness cycle
