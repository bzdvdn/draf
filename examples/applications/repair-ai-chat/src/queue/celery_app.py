"""Celery application for the repair-ai-chat background jobs.

Run the worker and the beat scheduler from compose::
...
interval so the RAG catalog picks up changed price lists automatically.
"""
...
celery_app.conf.beat_schedule = {
    "reingest-catalog-every-5-min": {
        "task": "repair_ai_chat.reingest_catalog",
        "schedule": 300.0,
    },
}
# Beat persists its schedule DB to disk; default to the example's ``data/``
...
@celery_app.task(name="repair_ai_chat.reingest_catalog")
def reingest_catalog(force: bool = False) -> dict:
    """Re-embed the material catalog when the seed CSVs changed on disk."""
