"""Deep research: plan questions, search the web, synthesize, review.

A research pipeline inspired by the "deep research" agents (LangGraph
...
- ``planner`` breaks the topic into a JSON object of research questions;
  ``extract_topics`` pulls the array into ``state["topics"]``.
- ``research_agent`` is a ReAct agent that searches the web and fetches
  pages until it can answer, then writes its notes.
- ``writer`` synthesises a report; ``reviewer`` returns a structured JSON
  review; on ``needs_work`` ``fix`` rewrites the report and the review
  loop repeats.
...
Usage:
    python examples/deep_research/graph.py
"""
...
def build_graph(model: str, live: bool) -> Graph:
    tools = (
        [FakeWebSearch(), FakeWebFetch()]
        if not live
        else [
            __import__(
    ...
    return Graph(
        ...
        edges=[
            Edge("planner", "extract_topics"),
            Edge("extract_topics", "research_agent"),
            Edge("research_agent", "tool_exec", condition="_tool_call_name!="),
            Edge("tool_exec", "research_agent"),
            Edge("research_agent", "writer"),
