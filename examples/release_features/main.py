...
1. ``teff validate`` equivalent — workflow YAML is checked before running.
2. ``run_eval`` — scores the workflow against a small dataset (exact mode).
3. Typed error hierarchy — catching ``teff.ConfigError`` instead of ``KeyError``.
...
def demo_custom_pricing() -> None:
    ...
    from teff.trace import clear_pricing, load_pricing, model_pricing
...
def main() -> None:
    ...
    demo_custom_pricing()
    print(f"\npackage version: teff {teff.__version__}")
