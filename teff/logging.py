...
Teff ships an operational log stream over the standard :mod:`logging`
...
_run_id: ContextVar[str] = ContextVar("teff_run_id", default="")
_session_id: ContextVar[str] = ContextVar("teff_session_id", default="")
_node_id: ContextVar[str] = ContextVar("teff_node_id", default="")
_node_type: ContextVar[str] = ContextVar("teff_node_type", default="")

#: Short env var that selectss the default level for
#: :func:`configure_logging` when it is not passed explicitly.
LOG_LEVEL_ENV = "TEFF_LOG_LEVEL"

#: Cap for LLM request/response payloads logged at DEBUG (characters).
LLM_LOG_CHARS_ENV = "TEFF_LOG_LLM_CHARS"

# Standard logging attribute names plus the teff contextvars.  These are
...
class _DrafOnlyFilter(logging.Filter):
    """Let only ``teff``-namespaced records reach the handler."""

    def filter(self, record: logging.LogRecord) -> bool:
        return record.name == "teff" or record.name.startswith("teff.")
