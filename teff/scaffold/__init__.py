"""Project templates + ``teff new`` scaffolding.

``teff new <name> --template <kind> [--with postgres,rag,celery]`` renders
a runnable teff app from one of three templates:

* ``fastapi`` — the default.  A FastAPI service: supervisor ``Flow`` built
  on :meth:`teff.flow.Flow.route`, tool set, durable sessions, API-key auth
  and a debug ``cli.py``.
...
def new_project(
    ...
    *,
    ...
    """Scaffold a new teff app from a template (+ optional variants).
    ...
    if manifest.name == "fastapi":
        _ensure_api_key(target)
    return target


def _ensure_api_key(target: Path) -> None:
    """Write a fresh random ``TEFF_API_KEY`` into the project's local ``.env``.

    The scaffold's auth is fail-closed: with no key configured every route
    returns 401.  Generating a random key here means a fresh project is
    usable immediately but never runs open.  ``.env`` is git-ignored by the
    scaffold, so the key is not committed.
    """
    env_file = target / ".env"
    if env_file.exists() and "TEFF_API_KEY=" in env_file.read_text(encoding="utf-8"):
        return
    with env_file.open("a", encoding="utf-8") as fh:
        fh.write("\n# Generated by `teff new`; required by the fail-closed API gate.\n")
        fh.write(f"TEFF_API_KEY={secrets.token_urlsafe(32)}\n")
