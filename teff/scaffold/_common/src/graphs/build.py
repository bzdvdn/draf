"""Supervisor graph builder — the minimal ``teff`` pattern.
...
1. ``Flow.route("next_agent", ...)`` — a supervisor loop: the decider node
   (:class:`teff.node.Supervisor`, added via :meth:`teff.flow.Flow.supervisor`)
...
2. :func:`teff.flow.agent_step` — the framework helper wrapping one agent
