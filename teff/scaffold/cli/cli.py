...
:class:`~teff.assistant.Assistant`, so sessions are durable and
tokens stream live.
...
app = typer.Typer(
    name="{{project_slug}}",
    help="Terminal-first teff app for {{PROJECT_NAME}}.",
...
def _logging(
    log_level: str | None = typer.Option(
        None, "--log-level", help="teff log level (DEBUG/INFO/WARNING/ERROR)"
    ),
    log_format: str = typer.Option(
        "text", "--log-format", help="teff log format: text or json"
    ),
) -> None:
    """Configure teff's operational logging."""
...
def run(
    ...
    asyncio.run(_stream(container.assistant, session, message))
