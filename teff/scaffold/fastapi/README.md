...
A production teff app scaffolded with `teff new {{project_slug}}`, organized
...
(`TEFF_*` vars) or a local `.env` file:

```
TEFF_PROVIDER=ollama
TEFF_MODEL=llama3.1:8b
TEFF_API_KEY=secret            # empty = auth disabled
TEFF_HOST=127.0.0.1
TEFF_PORT=8000
```
...
```
# 1. install the package (teff must be importable)
uv sync
...
- `TEFF_TRACES_DB` — where the traces are stored (default `data/traces.db`)
- `TEFF_TRACES_PREFIX` — URL prefix for the dashboard (default `/obs`)
- mount it in your own app with
  `teff.observability.attach_dashboard(app, exporter, prefix=...)`

## Template placeholders

Rendered by `teff new`: `{{PROJECT_NAME}}`, `{{project_slug}}`,
`{{ProjectName}}`.
