...
class TestReActAgent:
    ...
    async def test_agent_responds_directly(self, monkeypatch):
        from draf.builtin.agent import ReActAgent
        ...
        g = Graph(
            nodes={"agent": ReActAgent({"model": "gpt-4", "input_key": "input", "output_key": "output"})},
            edges=[],
            entry_point="agent",
        )
    ...
    async def test_agent_tool_cycle(self, monkeypatch):
        ...
        g = Graph(
            nodes={
                "agent": ReActAgent({"model": "gpt-4", "input_key": "input", "output_key": "output"}),
                "tool": ToolExec({}),
            },
            edges=[
        ...
        r = await g.run(state={"input": "make it uppercase"}, tools=[UpperTool()], max_iterations=5)
    ...
    async def test_agent_loop_exceeds_max_iterations(self, monkeypatch):
        ...
        async def mock_post(*a, **kw):
            class MockResponse:
                def raise_for_status(self): pass
                def json(self): return tool_response
            return MockResponse()
        ...
        with pytest.raises(RuntimeError, match="max_iterations"):
            await g.run(state={"input": "do it"}, tools=[AlwaysTool()], max_iterations=3)
...
class TestFlowReact:
    ...
    async def test_flow_react_direct_response(self, monkeypatch):
        ...
        async def mock_post(*a, **kw):
            class MockResponse:
                def raise_for_status(self): pass
                def json(self): return {"choices": [{"message": {"content": "direct answer"}}]}
            return MockResponse()
        ...
        flow = Flow("test")
        flow.react(model="gpt-4", system="You are helpful.", input_key="query", output_key="answer")
        g = flow.compile()
    ...
    async def test_flow_react_tool_cycle(self, monkeypatch):
        ...
        tool_resp = {
            "choices": [{
                "message": {
                    ...
                    "tool_calls": [{
                        "id": "call_1",
                        "function": {"name": "reverse", "arguments": '{"text": "hello"}'},
                    }],
                },
            }],
        }
        ...
        g = flow.compile()
        r = await g.run(state={"query": "reverse it"}, tools=[ReverseTool()], max_iterations=10)
